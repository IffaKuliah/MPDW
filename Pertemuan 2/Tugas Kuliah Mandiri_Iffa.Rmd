...
```{r}
ipm_bdg = read.xlsx("C:\\Users\\MUTHI'AH IFFA\\Downloads\\IpM BDG IFFA.xlsx", sheet = 1)
ipm_bdg$Tahun <- as.integer(ipm_bdg$Tahun)
View(ipm_bdg)
```

```{r}
model_ipm = lm(IPM ~ Tahun, data = ipm_bdg)
summary(model_ipm)
```
...
# regresi linear
model_db = lm(IPM ~ Tahun, data = ipm_bdg)
...
```{r}
model = lm(IPM ~ Tahun, data = ipm_bdg)
...
plot(res, type = "b", main = "Residuals vs Observation Order",
...
```{r}
acf(ipm_bdg, main = "Autocorrelation Function (ACF)")
```
...
model_awal <- lm(IPM ~ Tahun, data = ipm_bdg)
summary(model_awal)
```
...
plot(ipm_bdg$Tahun, sisaan_awal, type="o", pch=20, col="red",
...
```{r}
ipm.trans = ipm_bdg$IPM[-1] - ipm_bdg$IPM[-11]*rho

tahun.trans = ipm_bdg$IPM[-1] - ipm_bdg$IPM[-11]*rho
...
```{r}
model_hl = hildreth.lu(ipm_bdg$IPM, ipm_bdg$Tahun, rho = 0.8)
summary(model_hl)
```

```{r}
#Transformasi Balik
cat("y = ", coef(model_hl)[1]/(1-0.8), "+", coef(model_hl)[2],"x", sep = "")
```
...
data.frame(
  Metode = c("Model Awal (Sakit)", "Cochrane-Orcutt (Sakit Banget)", "Hildreth-Lu (Sehat)"),
